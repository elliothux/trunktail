...
public func listImages(completion: FFICompletion) {
    Task {
        do {
            ...
            try await withThrowingTaskGroup(of: ImageFullInfo.self) { group in
                for img in clientImages {
                    group.addTask {
                        ...
                        return ImageFullInfo(description: img.description, index: index, manifest: manifest, config: config)
                    }
                }
            ...
            let result = FFIResult.from(data: infos)
            completion(result.toJSON())
        } catch {
            let result = FFIErrorResult.from(msg: error.localizedDescription)
            completion(result.toJSON())
        }
    }
}
